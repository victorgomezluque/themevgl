...
function enqueue_custom_block()
{
	...
	wp_enqueue_script(
		'text-image-style',
		get_stylesheet_directory_uri() . '/widgets-gutenberg/text-image/editor.js',
		array('wp-blocks', 'wp-editor', 'wp-element'),
		filemtime(get_stylesheet_directory() . '/widgets-gutenberg/text-image/editor.js'),
		true
	);

	wp_enqueue_style(
		'text-image-style',
		get_stylesheet_directory_uri() . '/widgets-gutenberg/text-image/style.css',
		array(),
		filemtime(get_stylesheet_directory() . '/widgets-gutenberg/text-image/style.css')
	);
